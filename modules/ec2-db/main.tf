...
docker exec -i oracle-xe bash -lc \
  "sqlplus ${var.db_user}/${var.db_password}@XEPDB1" < "$1"
RUNSQL
...
# Run export
docker exec -it oracle-xe expdp ${var.db_user}/${var.db_password}@XEPDB1 \
  schemas=${var.db_user} \
  directory=DATA_PUMP_DIR \
  dumpfile=${var.db_user}_export.dmp \
  logfile=${var.db_user}_export.log \
  version=$VERSION

# Move files to accessible location
docker exec oracle-xe sh -c '
  mv /opt/oracle/admin/XE/dpdump/*/${var.db_user}_export.dmp /opt/oracle/admin/XE/dpdump/${var.db_user}_export.dmp 2>/dev/null || true
  mv /opt/oracle/admin/XE/dpdump/*/${var.db_user}_export.log /opt/oracle/admin/XE/dpdump/${var.db_user}_export.log 2>/dev/null || true
'
...
docker exec -it oracle-xe bash -lc \
  'sqlplus ${var.db_user}/${var.db_password}@XEPDB1 <<SQL
SELECT * FROM users;
exit
SQL'
CHECKSTATUS
    ...
    cd /home/ubuntu/oracle_xe
    docker exec -i oracle-xe bash -lc \
      "sqlplus ${var.db_user}/${var.db_password}@XEPDB1" < seed.sql || {
